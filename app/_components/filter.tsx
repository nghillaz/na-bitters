...
export function Filter(props: {
  ...
  return (
    <div
      ...
      className={classList(["filter", isExpanded ? "expanded" : ""])}
      ref={filterEl}
    >
      <div
        className={classList([
          "filterIcon",
          "highlightable",
          props.selections.length && !props.disabled && !isExpanded
            ? "highlighted"
            : "",
        ])}
      >
      ...
      <div
        ref={dropdownEl}
        className={classList(["dropdown", isExpanded ? "visible" : ""])}
      >
        {props.options.map((o) => (
          <DropdownOption
            option={o}
            key={o.key}
            onSelect={() => toggleOption(o)}
            selected={props.selections.indexOf(o.key) !== -1}
          />
        ))}
      </div>
    </div>
...
function DropdownOption(props: {
  ...
  onSelect: MouseEventHandler & KeyboardEventHandler;
  selected: boolean;
}) {
  return (
    <Button
      ...
      content={
        <>
          <Image
            ...
            height={20}
            alt=""
            className={classList(["icon", props.selected ? "remove" : ""])}
          />
